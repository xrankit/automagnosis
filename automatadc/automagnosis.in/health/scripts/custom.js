...
function sticky_relocate() {
    if($(window).width() >= 992){
        ...
        var div_bottom= window_top + 350;
        var footer_top = $('#footer').offset().top;
        if(window_top > div_top && div_top < (footer_top - 410)){
            if(div_bottom > footer_top){
                $('#sticky').removeClass('stick');
                $('#sticky').css('position','absolute');
                $('#sticky').css('top',footer_top - 410);
            }else{
                $('#sticky').css('position','');
                $('#sticky').css('top','');
...
//------------------------------------------------------------------------------------	
     ...
     $('#contact_form').submit(function() {
        var json = {};
        json.name = $('#contact_name').val();
        json.phone = $('#contact_phone').val();
        json.email = $('#type_form').val();
        json.age = $('#contact_age').val();
        json.gender = $("input[name=gender]:checked").val();
        json.urlstring = $('#querystring').val();
        json.identifier = "abcdefgh";
        if($(this).valid()){
            $('#contact_submit').button('loading'); 
            var action = $(this).attr('action');
            $.ajax({
                url: location.protocol+"www.docsapp.in/health/salesLeads/createSalesLead",
                type: 'POST',
                data: getformurlencoded(json),
                success: function() {
